...
<div class="main p-3">
    ...
    <div class="container mt-5">
        ...
        <select id="filter" name="filter">
            ...
            <option value="notdeliver">Not delivered</option>
...
<script>
    ...
    orders.forEach(function(ord, index) {
        ...
        let photoUrl = `${baseUrl}/${ord.order.order_maker.photo}`;
        ...
        table +=`   <tr>
                    ...
                    <td>${ord.order.order_maker.account_user.email}</td>

                    <td>${ord.order.address}</td>
                    <td>${ord.order.order_maker.account_user.number}</td>

                    <td>${ord.order.zip}</td>
                    <td>${ord.order.city}</td>
                    <td>${ord.order.get_state.name}</td>
                    <td>${ord.order.get_country.name}</td>
                    <td>${ord.tracking_number}</td>
                    <td><p style="color: ${ord.is_deliver === null ? 'red' : 'green'};"> 
                        ${ord.is_deliver === null ? "Not Yet Delivered" : "Delivered"}
                    </p></td>

                    <td>
                    ${ord.is_deliver === null ? 
        `<a href="#" class="make-deliver" id="make-deliver" data-id="${ord.id}"><button class="btn btn-primary">Make Deliver</button></a>` : 
        `<a href="#" class="remove-record"><button class="btn btn-danger">Remove Record</button></a>`
    }
    </td>
    ...
    $(document).on('click', '#make-deliver', function(e) {
        e.preventDefault(); // Prevent page reload
        // Your logic to mark the order as delivered
        let id=$(this).attr("data-id");
        console.log(id);
        
        let a=confirm('Order marked as delivered!');
        // console.log(a);
        if(a){
            $.ajax({
            url: "/admin/makedelivery",  // Replace with your API endpoint
            type: "GET",
            data: { id },  // Send data-id as a query parameter
    ...
    // Event listener for "Remove Record" button
    $(document).on('click', '.remove-record', function(e) {
        e.preventDefault(); // Prevent page reload
        // Your logic to remove the record
