...
<section id="product_add">
                ...
                <div class="upload_img_flex flex">
                    ...
                    <div class="colR">
                        <div class="upload_outer_blk">
                            ...
                            <div class="col_show" id="newImage">
                                {{-- <div class="img_show" id=newImage>
                                    <div class="inner">
                            ...
                            <div class="img_show" data-index="{{$image->id}}">
                                <div class="inner">
                                    <img src="{{asset($image->photo_path)}}" alt="Product Photo">
                                    <span class="x_btn"
                            ...
                            <div class="col_show " id="exsisting_image">
            ...
            <div class="block_item">
                ...
                <div class="fields_blk">
                    <div class="flex">
                        ...
                        <div class="field_col">
                            <div class="form_blk">
                                <input type="text" name="title"  id="title" value="{{$product->title}}"  class="input">
                        ...
                        <div class="field_col">
                            <div class="form_blk wide_5">
                                <select name="brand" class="input">
                                    ...
                                    <option value="{{$brands['id']}}" {{$brands->id == $product->brand->id ? "selected" : ""}}>{{$brands['name']}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="field_col">
                            <div class="form_blk wide_5">
                                <select name="category" class="input">
                                    ...
                                    <option value="{{$categories['sub_id']}}"

                                    {{$categories['sub_id'] == $product->subCategory->sub_id ? "selected":""}} 
                    ...
                    <div class="flex">
                        ...
                        <div class="field_col">
                            <div class="form_blk">
                                <textarea name="discription" id="" rows="10" class="input">{{$product->discription}}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="block_item">
                ...
                <div class="fields_blk">
                    <div class="flex">
                        ...
                        <div class="field_col">
                            <div class="form_blk wide_5">
                                <input type="text" name="price" id="" value="{{$product->price}}" placeholder="0" class="input price_field">
                    ...
                    <div class="flex">
                        ...
                        <div class="field_col">
                            <div class="form_blk wide_5">
                                <input type="text" name="quantity" id="" value={{$product->quantity}} class="input">
                            </div>
                        </div>
                    </div>
            ...
            <div class="block_item">
                ...
                <div class="fields_blk">
                    ...
                    <div class="flex">
                        ...
                        <div class="field_col">
                            <div class="check_blk">
                                {{-- @foreach ($product->productSize as $sizee)
                                ...
                                ${sizee.includes('XS') ? `checked="true"` :"" } 
                                <div class="lbl_btn">
    ...
    <div class="lbl_btn
    @if ($product->productSize->contains('size', $size)) active @endif
    "
    ...
    >
        <input 
            ...
            name="size[]" 
            value="{{ $size }}" 
            id="size{{ $loop->index + 1 }}" 
            class="hidden"
            {{-- Check if this size exists in the product_size collection --}}
            @if ($product->productSize->contains('size', $size)) checked="true" @endif
        >
...
// Refresh the previews
        ...
        // let index = $(this).closest('.img_show').index();
        // console.log(index)
        ...
        reader.onload = function(e) {
            $(`
                <div class="img_show" data-index="${index}">
                    <div class="inner">
                        <img src="${e.target.result}" alt="Product Photo">
                        <span class="x_btn closebtn remove-new-image"></span>
                    </div>
                </div>
            `).appendTo("#newImage");
...
$(document).ready(function() {
    ...
    //yahn krty hain kam apna
    var currentUrl = window.location.href;

// URL se product ID nikaalna (jo ke last part hai)
    var productId = currentUrl.split('/').pop();

    console.log(productId);  // Output: 38
    ...
    $.ajax({
        url: "{{ route('user.editProduct', '') }}/" + productId,   // URL to fetch data from
        type: 'GET',  // HTTP method type
        dataType: 'json',  // Expected data type from the server
        success: function(response) {
            // console.log(response);  // Server se jo data aayega, wo console pe print hoga

            // console.log(response.product) 
            let pho = response.product.product_photos.map(item => item.photo_path);
            console.log(pho)
    ...
    // let images = product.product_photos; // Assuming photos is an array of images
            let existingPhotos = response.product.product_photos;
            let exsistingColors=response.product.product_colors;
    ...
    <div class="outer_blk_color">
        <!-- Selected colors will be appended here -->
        <div class="color_blk" style="background-color:${color.color};">
            <span class="x_btn" onclick="removeColor(this)"></span>
        </div>
    ...
    //         let newImagesArray = []; // Store new images
    // // let images = product.product_photos; // Assuming photos is an array of images
    //         let existingPhotos = producting.product_photos;
    ...
    //           loadExistingImages(existingPhotos);
        ...
        error: function(xhr, status, error) {
            console.log('Error: ' + error);  // Agar koi error ho to yeh message show hoga
        }
    });



    //exsisting imges ko deal kryn gy,
    function loadExistingImages(images) {
            ...
            $(`
                <div class="img_show" data-index="${index}">
                    <div class="inner">
                        <img src="{{asset('/')}}${image.photo_path}" alt="Product Photo">
                        <span class="x_btn remove-existing-image"></span>
                       
                    </div>
                </div>
            `).appendTo("#exsisting_image");
            // closebtn
            // $('.col_show').removeClass('newImage');
         existingImagesArray.push(image); // Push to the array
         $('.col_show').removeClass('.col_show');
         });
        ...
        let index = $(this).closest('.img_show').data('index');
       
        existingImagesArray.splice(index, 1); // Remove the image from the array
        $(this).closest('.img_show').remove(); // Remove from DOM
    ...
    existingImagesArray.forEach(function(image) {
            formData.append('existing_photos[]', image.id);  // Send image IDs to keep
                    ...
                    $.ajax({
                        ...
                        url: "{{ route('user.updateProduct', '') }}/" + productId,   // URL to fetch data from
